...
// #define WAVE_SAMPLE_OFFSET_DIST 1.0
#define WAVE_SAMPLE_OFFSET_DIST 1.0
...
float noiseScale = 1.0 / 128.0;
// float noiseScale = 1.0 / 32.0;
...
float psnoise_fbm (in vec2 st, vec2 per) {
    // Initial values
    float value = 0.0;
    float amplitud = 0.5;
    float frequency = 0.0;
...
vec4 update(vec2 uv) {
  ...
  for (int i = 0; i < 4; i ++) {
    vec4 waveSample = texture(iChannel0, uv + d[i] / iResolution.xy * WAVE_SAMPLE_OFFSET_DIST);
  ...
  // vec2 noiseSign = sign(noiseVal);
  // noiseVal = abs(noiseVal);
  // noiseVal = clamp(noiseVal, 0.0, 1.0);
  // noiseVal *= noiseSign;
  // noiseVal = fract(noiseVal);

  color = vec4(
    gradient.x * WAVE_GRADIENT_MULT + noiseVal.r * NOISE_MULT, 
    gradient.y * WAVE_GRADIENT_MULT + noiseVal.g * NOISE_MULT, 
    0.0, 
    1.0);
